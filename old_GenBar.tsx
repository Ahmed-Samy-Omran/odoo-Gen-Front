...
const ARABIC_JSON_ERROR = '╪º┘ä╪▒╪¼╪º╪í ╪Ñ╪»╪«╪º┘ä ┘å╪╡ JSON ╪║┘è╪▒ ┘ü╪º╪▒╪║ ┘ê╪╡╪º┘ä╪¡.';
...
export const GenBar: React.FC<GenBarProps> = ({ onGenerate, onTryDemo, resetKey, initialMessages = [], onMessagesChange, jobId }) => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : '┘ü╪┤┘ä ╪º┘ä╪º╪¬╪╡╪º┘ä ╪¿╪º┘ä┘Ç AI');
    } finally {
      setIsChatting(false);
    }
  };
  ...
  const handleGenerate = () => {
    ...
    if (mode === 'json') {
      ...
      setError('┘â┘à┘ä ╪º┘ä┘à╪¡╪º╪»╪½╪⌐ ┘à╪╣ ╪º┘ä┘Ç AI ┘ä╪¡╪» ┘à╪º ┘è┘é┘ê┘ä┘â ╪Ñ┘å ╪º┘ä┘à╪¬╪╖┘ä╪¿╪º╪¬ ╪¼╪º┘ç╪▓╪⌐.');
      return;
    }
      ...
      <div className="bg-[#0b0b0d] text-white rounded-3xl px-4 py-5 shadow-2xl backdrop-blur-md">
        ...
        {mode === 'text' && readyToGenerate && (
          <div className="mb-3 mx-2 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-2 text-xs text-emerald-300">
            ╪º┘ä┘à╪¬╪╖┘ä╪¿╪º╪¬ ╪¼╪º┘ç╪▓╪⌐ ΓÇö ╪º╪╢╪║╪╖ Generate ┘ä╪¿┘å╪º╪í ╪º┘ä┘à┘ê╪»┘è┘ê┘ä
          </div>
        )}
          ...
          <div className="flex items-center gap-4 flex-1 bg-transparent rounded-2xl px-5 py-4">
            ...
            <textarea
              ...
              placeholder={
                mode === 'json'
                  ? '{ "prompt": "..." } or { "modules": [...] }'
                  : '╪╡┘ü ╪º┘ä┘à┘ê╪»┘è┘ê┘ä ╪º┘ä┘ä┘è ╪╣╪º┘è╪▓┘ç... Enter ┘ä┘ä╪Ñ╪▒╪│╪º┘ä'
              }
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-2 text-sm text-white/70 leading-6">
                    ...
                    send a message. Short replies like &quot;╪º┘è┘â&quot; or &quot;ok&quot; are acknowledged but
                    won&apos;t start generation. When requirements are complete, you&apos;ll see a green
                    ready badge ΓÇö then click Generate.
                  </p>
                </div>
                <button
                  type="button"
